...
class StockQuant(models.Model):
    ...
    def check_negative_qty(self):
        ...
        check_negative_qty = (config['test_enable'] and self.env.context.get('test_stock_no_negative')) or (not config.options.get('test_enable', False))
        if not check_negative_qty:
            return
        for quant in self:
            if (float_compare(quant.quantity, 0, precision_digits=p) == -1 and
                    quant.product_id.type == 'product' and quant.location_id.usage == 'internal' and
